...
import { Graph, Point, BoundingBox } from '../core/types';
...
export function forceDirectedLayout(
  ...
  const cleanOptions: ForceDirectedLayoutOptions = {};
...
function initializeNodes(
  graph: Graph,
  opts: Required<ForceDirectedLayoutOptions>
): LayoutNode[] {
  ...
  for (const [id, node] of graph.nodes) {
    ...
    const label = (node.attributes.label as string) || id;
 ...
 */
function routeEdges(graph: Graph): void {
...
function getNodeBoundaryPoint(
  node: { x?: number; y?: number; width?: number; height?: number; attributes: { shape?: unknown } },
 ...
 */
function normalizeAndCalculateBoundingBox(
  graph: Graph,
  margin: number
): BoundingBox {
  let minX = Infinity;
...
export function circularLayout(
  graph: Graph,
  options: { radius?: number; margin?: number } = {}
): Graph {
  const result = cloneGraph(graph);
...
export function radialLayout(
  graph: Graph,
  options: { root?: string; levelSep?: number; margin?: number } = {}
): Graph {
  const result = cloneGraph(graph);
  const n = result.nodes.size;
  ...
  if (!rootId) {
    ...
    if (!rootId) {
      rootId = result.nodes.keys().next().value as string;
    }
  }
