...
function renderSVG(graph, options = {}) {
    ...
    if (graph.attributes.fontname) {
        opts.fontFamily = graph.attributes.fontname;
    }
    ...
    if (opts.background && opts.background !== 'transparent') {
        ...
        lines.push(`  <rect width="100%" height="100%" fill="${escapeXml(graph.attributes.bgcolor)}"/>`);
    }
    ...
    if (graph.attributes.label) {
        ...
        lines.push(`      ${escapeXml(graph.attributes.label)}`);
        lines.push(`    </text>`);
    }
    lines.push(`  </g>`);
    lines.push(`</svg>`);
 ...
 */
function renderCluster(subgraph, opts) {
    const bb = subgraph.boundingBox;
    const lines = [];
    const style = subgraph.attributes.style || '';
    const color = subgraph.attributes.color || 'black';
    const fillcolor = subgraph.attributes.fillcolor;
    const penwidth = subgraph.attributes.penwidth || 1;
    const label = subgraph.attributes.label;
    ...
    lines.push(`      />`);
    if (label) {
        const labelloc = subgraph.attributes.labelloc || 't';
        const labeljust = subgraph.attributes.labeljust || 'c';
...
function renderNode(node, opts) {
    ...
    const fontname = (node.attributes.fontname || opts.fontFamily);
    const fontsize = (node.attributes.fontsize || opts.fontSize);
    ...
    const url = (node.attributes.URL || node.attributes.href);
    if (url) {
 ...
 * Render a node shape
 */
function renderShape(shape, x, y, width, height, fill, stroke, strokeWidth, strokeDasharray, _attrs) {
 ...
 */
function polygonPoints(cx, cy, r, sides, startAngle = 0) {
    const points = [];
 ...
 * Render an edge
 */
function renderEdge(graph, edge, opts) {
    const lines = [];
    const style = (edge.attributes.style || '');
    const color = (edge.attributes.color || 'black');
    const penwidth = (edge.attributes.penwidth || 1);
    const fontcolor = (edge.attributes.fontcolor || color);
    const fontname = (edge.attributes.fontname || opts.fontFamily);
    const fontsize = (edge.attributes.fontsize || opts.fontSize);
    const strokeDasharray = style.includes('dashed') ? '5,5' : (style.includes('dotted') ? '2,2' : 'none');
    const dashAttr = strokeDasharray !== 'none' ? ` stroke-dasharray="${strokeDasharray}"` : '';
    // Determine arrow markers
    const dir = (edge.attributes.dir || (graph.type === 'digraph' ? 'forward' : 'none'));
    const arrowhead = (edge.attributes.arrowhead || 'normal');
    const arrowtail = (edge.attributes.arrowtail || 'normal');
    let markerEnd = '';
    let markerStart = '';
    ...
    const tooltip = edge.attributes.tooltip;
    const url = (edge.attributes.URL || edge.attributes.href);
    if (url) {
 ...
 */
function getArrowMarkerId(type) {
    switch (type) {
        case 'none': return '';
 ...
 */
function splinesToPath(splines) {
    if (splines.length === 0)
        return '';
    const parts = [];
